...
vstup = 'Добро пожаловать😊\n Это твой персональный помощник✨\n Первый БОТ - тренажер🤖 «ЕГЭ-Русский язык»🔥🔥🔥\n Просто выбери номер задания в тестах ЕГЭ и начни решать!'
            ...
            if messages["count"] >= 1:
                ...
                if not mass_fg[mass_id.index(id)]:
                    if body.lower() == 'привет':
                        ...
                        vkontakte.method("messages.send",
                                         {"peer_id": id, "message": 'Давай порешаем, выбери задание из списка',
                                          "random_id": random.randint(1, 124323125345),
                        ...
                        if proverka(kolvo_zad[mass_id.index(id)][number_active_zad[mass_id.index(id)]],number_active_zad[mass_id.index(id)])==0:
                            ...
                            print(kolvo_zad[mass_id.index(id)][number_active_zad[mass_id.index(id)]])
                            print(number_active_zad[mass_id.index(id)], "++++")
                            vkontakte.method("messages.send",
                        ...
                        if body.lower() == 'закончить':
                            ...
                            vkontakte.method("messages.send",
                                             {"peer_id": id, "message": 'Ответ неверный',
                                              ...
                                              "keyboard": keyboard1.get_keyboard()})
                            print(count_id)
                            print(count_id[mass_id.index(id)])
